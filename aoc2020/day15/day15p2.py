...
def evaluate(starting, *, iterations: int):
  history = {v: [turn + 1] for turn, v in enumerate(starting)}

  last_spoken = starting[-1]
  for turn in range(len(starting) + 1, iterations + 1):
    if turn % 1_000_000 == 0:
      print(f'{turn:,}')
    ...
    turns_spoken = history.get(last_spoken, [])
...
if __name__ == '__main__':
  ...
  input = (15,5,1,4,7,0)
  with timed(str(input)):
    print(evaluate(input, iterations=30_000_000))
